...
import addMoreIcon from "@/public/addMore-icon.svg";
...
export default function FlightFilter() {
  ...
  const [fromPanelToggle, setfromPanelToggle] = useState(false);
  const [toPanelToggle, setToPanelToggle] = useState(false);
  ...
  const [numSelectedCities, setNumSelectedCities] = useState([
  ...
  // Watch the form values for creating the search URL
          ...
          {actionButton.map((button, i) => (
            <ButtonCustom
              ...
              onClick={() =>
                i === 2
                  ? () => {
                      setSelectedTicketType(button.type);
                      handleAddRow();
                    }
                  : () => {
                      setSelectedTicketType(button.type);
